...
/**
 * http://robowiki.net/wiki/SuperRamFire implementation only there are some differences are made so the robot will behave deterministic
 * such as introducing Random generator with fixed seed for every random desicion and changing static variables to 
 * nonstatic so previous runs won't affect them
 ...
 *
 */
public class DeterministicRamFire extends AdvancedRobot {
	...
	// Variables
	// Made nonstatic so previous runs won't affect them
	double dir=1;
	...
	public void onScannedRobot(ScannedRobotEvent e){
		...
		if(enemyEnergy>(enemyEnergy=e.getEnergy())){
			...
			if(generator.nextDouble()>200/e.getDistance()){
				dir=-dir;
			}
		}
